...
public class ControllerAddMember {
    ...
    /*
        Handles adding member to database. If requirements are met, the user will be uploaded,
        if not, an Alert will be shown
    */
    public void handleConfirm() {
        if(verifyDataForUpload() == null) {
            Client clientModel = new Client(-1, fullname.getText(), email.getText(), pin.getText(),
                    (genderM.isSelected() ? "Male" : "Female"),
                    "No", "No", "No", "No");

            if(clientModel.upload()) {
                fullname.setText("");
                ...
                alert.setHeaderText("Member already exists");
                alert.showAndWait();
            }
        }
        else
            verifyDataForUpload().showAndWait();
    }

    /*
        Changes scene from addMember to dashboard
    */
